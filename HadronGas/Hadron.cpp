...
double Hadron::hbarC     = 0.19733;
int    Hadron::trunOrder = 10;

Hadron::Hadron(int pdgCode_in, string name_in, double mass_in,
...
stable(0),
mu(0.0),
sign((baryon_in==0)?-1:1), // determine Bose/Fermi statistic for Hadron
channelIdx(0),
yield(0.0),
stableYield(0),
dPoverTdmu(0.0),
...
Hadron::Hadron(const Hadron & source)
:
...
mu(source.mu),
sign(source.sign), // determine Bose/Fermi statistic for Hadron
...
Hadron & Hadron::operator=(const Hadron & source)
{
  if (this!=&source)
    {
    pdgCode  =  source.pdgCode;
...
void Hadron::addResonanceDecays(double branchratio,
                                int Npart,
                                int* decayChannelHadrons)
{
  // add resonance decay channel for Hadron
...
// ======================================================================
// Return the corresponding anti-Hadron' Monte-Carlo value
// ======================================================================
int Hadron::getAntiHadronPdgCode()
{
...
void Hadron::calculateChemicalPotential(double muB, double muS)
{
  mu = muB*baryon + muS*strange;
}

//void Hadron::createDecayProbabilityArray(int nAll, int nStable)
//{
////  decayStableProbability      = new double[nStable];
////  decayStableProbabilityPairs = new double*[nStable];
//  decayProbability            = new double[nAll];
//  decayProbabilityPairs       = new double*[nAll];
//  for (int i = 0; i < nAll; i++)
//  {
//  if (i<nStable)
//    {
//    decayStableProbability[i] = 0.0;
//    decayStableProbabilityPairs[i] = new double[nStable];
//    }
//  decayProbability[i] = 0.0;
//  decayProbabilityPairs[i] = new double[nAll];
//  for (int j = 0; j < nAll; j++)
//    {
//    decayProbabilityPairs[i][j] = 0.0;
//    if (j<nStable)
//      {
//      decayStableProbabilityPairs[i][j] = 0.0;
//      }
//    }
//  }
//}

void Hadron::createDecayProbabilityArray(int nStable)
{
...
// ======================================================================
// Compute Hadron thermal yield at given T, mu
// ======================================================================
void Hadron::calculateParticleYield(double temperature,
                                          double muB,
                                          double muS)
{
...
// ======================================================================
// Compute Hadron Energy Density at given T
// ======================================================================
double Hadron::calculateEnergyDensity(double temperature)
{
  double results;
  results = 0.0;
...
// ======================================================================
// Compute Hadron (Partial) Pressure at given T
// ======================================================================
double Hadron::calculatePressure(double temperature)
{
  double results;
  results = 0.0;
...
// ======================================================================
// Compute Hadron (Partial) Entropy Density
// using the first law of thermodynamics
// at given T and mu
// ======================================================================
double Hadron::calculateEntropyDensity(double temperature)
{
...
// ======================================================================
// Compute Hadron dN/dmu at given T [1/GeV]
// ======================================================================
double Hadron::calculate_dndmu(double temperature)
{
  double results;
  results = 0.0;
...
// ======================================================================
// Compute Hadron 1/T dP/dmu at given T [1/(GeV fm^3)]
// ======================================================================
double Hadron::calculate_dPoverTdmu(double temperature)
{
  double results;
  results = 0.0;
...
// ======================================================================
// Compute Hadron d(e/T)/dmu [1/(GeV fm^3)]
// ======================================================================
double Hadron::calculate_deoverTdmu(double temperature)
{
  double results;
  results = 0.0;
  double prefactor = gSpin/(2*M_PI*M_PI)*pow(mass, 4);
...
// ======================================================================
// Compute Hadron first order derivative ds/dmu [1/(GeV fm^3)]
// ======================================================================
double Hadron::calculate_dsdmu(double temperature)
{
...
ostream & Hadron::printBasicProperties(ostream & os)
{
  //scientific
  os <<  fixed << setw(10) << setprecision(8) << pdgCode;         // Monte-Carlo number according PDG
  os <<  setw(20) << name;   // Hadron name
  os <<  scientific << setw(20) << setprecision(4) <<  mass;        // Hadron mass (GeV)
  ...
  os <<  fixed << setw(5) << setprecision(3) <<  stable;         // defines whether this Hadron is stable
...
ostream & Hadron::printCalcProperties(ostream & os)
{
  //scientific
  os <<  fixed << setw(10) << setprecision(8) << pdgCode;         // Monte-Carlo number according PDG
  os <<  setw(20) << name;   // Hadron name
...
// void Hadron::getBulkViscosityCoefficients(
