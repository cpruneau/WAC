...
collisionGeometry(collisionGeo)
{
  if (reportNoOps("AACollisionGenerator",getTaskName(),"CTOR"))
    ;
}

AACollisionGenerator::~AACollisionGenerator()
{
  if (reportNoOps("AACollisionGenerator",getTaskName(),"DTOR"))
    ;
}

void AACollisionGenerator::initialize()
{
  if (reportStart("AACollisionGenerator",getTaskName(),"initialize()"))
    ;
  nMax = 10000 ;
  ...
  if (reportEnd("AACollisionGenerator",getTaskName(),"initialize()"))
    ;
}

void AACollisionGenerator::execute()
{
  if (reportStart("AACollisionGenerator",getTaskName(),"execute()"))
    ;
  HeavyIonConfiguration * hc = (HeavyIonConfiguration *) getTaskConfiguration();
  nCollisions = collisionGeometry->nBinary; //get the number of binary collisions
  if (reportDebug("AACollisionGenerator",getTaskName(),"execute()")) cout << "Processing " << nCollisions << " collisions." << endl;
  Factory<Particle> * particleFactory = Particle::getFactory();
  ...
  if (reportDebug()) cout << "Generated Event " << getNEventProcessed() + 1 << ":" << i + 1 << endl;
  } 
