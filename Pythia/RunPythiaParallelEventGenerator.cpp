...
int main()
{
  ...
  long nEventsPerTree = 1000000; //make sure this divides the number of events
  int  nThreads    = 5;        // make sure this divides the number of events per tree
  int eventsPerThread = nEventsPerTree/nThreads;
  int nLoops = nEventsRequested/nEventsPerTree;
  cout << "Events per Thread: " << eventsPerThread << endl;
  TString outputFolder = getenv("OUTPUT_PATH");
  TFile * file = new TFile(outputFolder + "/PythiaEventTree2.root", "RECREATE");
  ...
  TString ** pythiaOptions = new TString* [50];
    ...
    TBranch * b = tree->Branch(branchname + (1), particles, 32000 , 0 );
    ...
    seed = ((seed  * (1)) % 90000000) * ((seed  * ( 1)) % 90000000) * ((seed  * (1)) % 90000000) * ((seed  * (1)) % 90000000) * ((seed  * (1)) % 90000000); //for differentiation of tasks created very soon after one another
    seed = seed % 90000000;
    cout << "Seed from thread " << 1 << " is " << seed << endl;
    ...
    for (int i = 0; i < nLoops; ++i)
    {
      ...
      TTree * tree = new TTree("EventTree" + i, "EventTree" );
